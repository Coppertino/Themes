...
@interface IADocumentWindowController ()
{
    NSMutableDictionary *_minimumViewRects;
    NSMutableDictionary *_loadedContent;
}
@end

@implementation IADocumentWindowController
...
- (void)windowDidLoad
{
    ...
    [self.themeSelectionButton setAutoresizingMask:NSViewMinXMargin | NSViewMinYMargin];
    [self.themeSelectionButton setFrameOrigin:NSMakePoint(NSMaxX([self.window.contentView frame]) - NSWidth(self.themeSelectionButton.frame) - 8,
                                                          NSMaxY([[self.window.contentView superview] frame]) - 24)];
    
    [[[[self window] contentView] superview] addSubview:self.themeSelectionButton];
    
    [self.statusView setAutoresizingMask:NSViewMaxXMargin | NSViewMaxYMargin];
    [[[[self window] contentView] superview] addSubview:self.statusView];
...
{
    NSViewController <IATabViewController> *viewController = self.selectedViewController;
    if (viewController)
...
- (void)setSelectedViewController:(NSViewController <IATabViewController> *)controller
{
...
- (NSToolbarItem *)toolbar:(NSToolbar *)toolbar itemForItemIdentifier:(NSString *)itemIdentifier willBeInsertedIntoToolbar:(BOOL)flag
{
    ...
    {
        id <IATabViewController> controller = [_viewControllers objectAtIndex:controllerIndex];
...
- (NSViewController <IATabViewController> *)viewControllerForIdentifier:(NSString *)identifier
{
...
- (NSViewController <IATabViewController> *)firstViewController {
    for (id viewController in self.viewControllers)
